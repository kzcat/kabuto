...
- **幅いっぱいに可変**: タイル幅は固定 27 桁ではなく、`列数 = max(1, termWidth / 最小タイル幅 24)` とし(ただし列数は表示銘柄数を超えない)、`タイル幅 = termWidth / 列数`(余り桁は左の列から 1 桁ずつ配分して合計が端末幅に一致するように)。スパークラインや名称はタイル幅に追従して伸びる
